...
public class QueryInterceptor1 implements Interceptor {
    ...
    public Object intercept(Invocation invocation) throws Throwable {
        ...
        BoundSql boundSql;
        if (args.length == 4) {
            System.out.println("QueryInterceptor1 拦截4参数Query方法");
            //4 个参数时
            boundSql = mappedStatement.getBoundSql(parameter);
            cacheKey = executor.createCacheKey(mappedStatement, parameter, (RowBounds) args[2], boundSql);
        } else {
            System.out.println("QueryInterceptor1 拦截6参数Query方法");
            //6 个参数时
        ...
        System.out.println("QueryInterceptor1 拦截方法:" + method);
        System.out.println("QueryInterceptor1 拦截sql:" + sql);
